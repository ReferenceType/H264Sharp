    ...
    public class H264Encoder : IDisposable
    {
        ...
        public int ConverterNumberOfThreads
        {
            ...
            set
            {
                ...
                H264NativeApi.SetParallelConverterEnc(encoder, value);
            }
        }
        ...
        public H264Encoder()
        {
            encoder = H264NativeApi.GetEncoder();
        }
        ...
        public H264Encoder(string ciscoDllName)
        {
            encoder = H264NativeApi.GetEncoder(ciscoDllName);
        }

        public TagEncParamExt GetDefaultParameters()
        {
            TagEncParamExt paramExt = new TagEncParamExt();
            H264NativeApi.GetDefaultParams(encoder, ref paramExt);
            return paramExt;
        }
        ...
        {
            return H264NativeApi.InitializeEncoder(encoder, width, height, bitrate, fps, (int)configType);
        }

        public int Initialize(TagEncParamBase param)
        {
            return H264NativeApi.InitializeEncoderBase(encoder, param);
        }

        public int Initialize(TagEncParamExt param)
        {
            return H264NativeApi.InitializeEncoder2(encoder, param);
        }

        /// <summary>
        /// Forces an intra frame with instant decoder refresh on next encode.
        /// This is used to refresh decoder in case of lost frames.
        /// Be aware that Intra frames are large.
        /// </summary>
        /// <returns></returns>
        public bool ForceIntraFrame()
        {
            var ret = H264NativeApi.ForceIntraFrame(encoder);
        ...
        {
            H264NativeApi.SetMaxBitrate(encoder, target);
        }
        ...
        {
            H264NativeApi.SetTargetFps(encoder, target);
        }
        ...
        public bool GetOption<T>(ENCODER_OPTION option, out T value) where T : struct
        {
            unsafe
            {
                ...
                if (value is bool)
                {
                    ...
                    {
                        int r = H264NativeApi.GetOptionEncoder(encoder, option, new IntPtr(&toSet));
                        var success = (r == 0);
                ...
                fixed (T* V = &value)
                {
                    int r = H264NativeApi.GetOptionEncoder(encoder, option, new IntPtr(V));
                    value = *V;
                    return (r == 0);
                }
            }
        ...
        public bool SetOption<T>(ENCODER_OPTION option, T value) where T : struct
        {
            unsafe
            {
                if (value is bool)
                {
                    bool v = (bool)((object)value);
                    byte toSet = v ? (byte)1 : (byte)0;
                    return H264NativeApi.SetOptionEncoder(encoder, option, new IntPtr(&toSet)) == 0;
                }
                return H264NativeApi.SetOptionEncoder(encoder, option, new IntPtr(&value)) == 0;
            }
        }

        /// <summary>
        /// Encodes image provided on Image data.
        /// </summary>
        /// <param name="im"></param>
        /// <param name="ed"></param>
        /// <returns></returns>
            ...
            unsafe
            {
                ...
                if (im.isManaged)
                {
                    fixed (byte* dp = &im.data[im.dataOffset])
                    {
                        ...
                        var success = H264NativeApi.Encode(encoder, ref ugi, ref fc);
                        ed = Convert(fc);
                        return success == 1;
                    }
                }
       ...
       /// <returns></returns>
        ...
        {
            var fc = new FrameContainer();
            var success = H264NativeApi.Encode1(encoder, ref YUV[0], ref fc);
            ed = Convert(fc);
            return success == 1;
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                H264NativeApi.FreeEncoder(encoder);
                disposedValue = true;
            }
        }
