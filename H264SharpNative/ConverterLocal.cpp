...
void Yuv420P2RGBDefault(unsigned char* dst_ptr,
    ...
    int numThreads) 
{
    if (width * height > minSize && numThreads > 1)
    {

        pool.For(0, numThreads, [width, height, &y_ptr, &u_ptr, &v_ptr, y_span, uv_span, &dst_ptr, dst_span, numThreads](int j)
            {
                int hi = height / 2;
    ...
    int numThreads)
{
    ...
    if (numThreads > 1 && (width * height > minSize)) 
    {

        pool.For(0, numThreads, [width, height, Y, U, V, Y_stride, UV_stride, RGB, RGB_stride, numThreads](int j)
            {
                int hi = (height / 2);
                int bgn = (hi / numThreads) * (j);
                int end = (hi / numThreads) * (j + 1);
...
// clang does amazing job on vectorization.
//------------------------------------BGRx->Yuv-------------------------------------------------
...
void BGRAtoYUV420Planar(const unsigned char* bgra, unsigned char* dst, const int width, const int height, const int stride, int numThreads)
{
    ...
    if (width * height > minSize && numThreads > 1)
    {
        pool.For(int(0), numThreads, [bgra, dst, width, height, stride, hi, numThreads](int j)
            {
                int bgn = ((hi / numThreads) * (j));
                int end = ((hi / numThreads) * (j + 1));
                if (j == numThreads - 1)
